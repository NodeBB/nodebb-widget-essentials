<div class="d-flex forum-stats flex-wrap mb-3 gap-4">
	<div class="d-flex flex-1 gap-4">
		<div class="flex-1">
			<div class="stats-card text-center {statsClass}">
				<h4 class="stats" title="{online}">{online}</h4>
				<span class="text-secondary">{{tx("global:online")}}</span>
			</div>
		</div>
		<div class="flex-1">
			<div class="stats-card text-center {statsClass}">
				<h4 class="stats" title="{users}">{users}</h4>
				<span class="text-secondary">{{tx("global:users")}}</span>
			</div>
		</div>
	</div>
	<div class="d-flex flex-1 gap-4">
		<div class="flex-1">
			<div class="stats-card text-center {statsClass}">
				<h4 class="stats" title="{topics}">{topics}</h4>
				<span class="text-secondary">{{tx("global:topics")}}</span>
			</div>
		</div>
		<div class="flex-1">
			<div class="stats-card text-center {statsClass}">
				<h4 class="stats" title="{posts}">{posts}</h4>
				<span class="text-secondary">{{tx("global:posts")}}</span>
			</div>
		</div>
	</div>
</div>
